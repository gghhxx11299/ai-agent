"""OpenRouter AI integration as fallback option"""
...
class OpenRouterIntegration:
    """OpenRouter AI integration for query analysis and response synthesis"""
...
{{
  ...
  "requiresCurrentData": true/false
        ...
        try:
            ...
            import logging
            logging.warning(f"Error analyzing query with OpenRouter: {e}")
    ...
    async def synthesize_response(self, query: str, data: dict) -> str:
        """
        Synthesize a final answer from aggregated data using OpenRouter
        ...
        prompt = f"""You are a friendly AI assistant having a natural conversation with a user. You have access to real-time data sources and can provide up-to-date, accurate information.
...
Provide your response in a natural, friendly way:"""
    ...
    async def answer_directly(self, query: str) -> str:
        """
        Answer query directly using OpenRouter's knowledge (when no external data is needed)
