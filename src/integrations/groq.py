"""Groq AI integration as fallback for Gemini"""
...
class GroqIntegration:
    """Groq AI integration for query analysis and response synthesis"""
...
{{
  ...
  "requiresCurrentData": true/false
        ...
        try:
            ...
            import logging
            logging.warning(f"Error analyzing query with Groq: {e}")
    ...
    async def synthesize_response(self, query: str, data: dict) -> str:
        """
        Synthesize a final answer from aggregated data using Groq
        ...
        prompt = f"""You are Groq, a friendly AI assistant having a natural conversation with a user. You have access to real-time data sources and can provide up-to-date, accurate information.
...
Provide your response in a natural, friendly way:"""
        ...
        try:
            ...
            import logging
            logging.error(f"Error synthesizing response with Groq: {e}")
    ...
    async def answer_directly(self, query: str) -> str:
        """
        Answer query directly using Groq's knowledge (when no external data is needed)
        ...
        """
        prompt = f"""You are a friendly and knowledgeable AI assistant. You specialize in agriculture, weather, regional information, and general knowledge. You're conversational, helpful, and provide accurate information.
...
Your response:"""
        ...
        try:
            ...
            import logging
            logging.error(f"Error in direct answer with Groq: {e}")
